...
export const generateConfirmationPDF = async (reservation: Reservation): Promise<void> => {
  // Crear el contenido HTML para la confirmación
  const confirmationHTML = createConfirmationHTML(reservation);
  ...
  try {
    ...
    // Descargar el PDF
    pdf.save(`Confirmacion_${reservation.customerName.replace(/\s+/g, '_')}_${format(new Date(), 'dd-MM-yyyy')}.pdf`);
  } finally {
    // Limpiar el elemento temporal
    document.body.removeChild(tempDiv);
  }
  ...
  return `
    <div style="max-width: 800px; margin: 0 auto; padding: 40px; font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
          <div>
            <strong>Nombre:</strong> ${reservation.customerName}<br>
      ...
      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
          ...
          <div>
            <strong>Huéspedes:</strong> ${reservation.adults} adultos${reservation.children8to15 > 0 ? `, ${reservation.children8to15} niños (8-15 años)` : ''}${reservation.childrenUnder7 > 0 ? `, ${reservation.childrenUnder7} menores de 7 años` : ''}<br>
      ...
      <div style="background: #fefce8; border: 1px solid #a16207; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
        ...
        <p style="margin: 0; color: #854d0e;">${reservation.notes}</p>
      </div>
      ` : ''}
